  ...
  <div ref="top">
    ...
    <div v-else class="columns is-mobile is-multiline is-relative">
      <div class="column is-10">
        <header ref="shortstories" class="card-header bg-ghostwhite">
          <h3 class="card-header-title">
            <b-icon icon="note-text-outline" class="has-text-info m-r-md"></b-icon>
            เรื่องสั้น
          </h3>
          <nuxt-link
            :to="{
              name: 'read-shortstories',
            }"
            title="ดูทั้งหมด"
        ...
        <div class="columns is-multiline is-mobile p-x-md p-t-md">
          <div
            v-for="(item, key) in shortstories"
            :key="key"
            class="column is-half-mobile is-one-fifth-desktop shine"
          >
            <nuxt-link :to="{name: 'read-shortstories-category',params: { category: item.slug }}" :title="item.name">
        ...
        <header ref="article" class="card-header bg-ghostwhite">
          <h3 class="card-header-title">
            <b-icon icon="book-outline" class="text-green m-r-md"></b-icon>
            บทความ
          </h3>
          <nuxt-link
            :to="{
        ...
        <div class="columns is-multiline is-mobile p-x-md p-t-md">
          ...
          >
            <nuxt-link :to="{name: 'read-article-slug',params: { slug: item.slug }}" :title="item.title">
      ...
      <div class="column is-2 p-l-none-mobile">
        <aside class="is-paddingless sticky">
          ...
          <ul class="menu-list">
            <li>
              <a @click="goto('shortstories')">
                <b-icon icon="note-text-outline" class="has-text-info" />
                <span class="is-hidden-mobile">เรื่องสั้น</span>
              </a>
            </li>
            <li>
              <a @click="goto('article')">
                <b-icon icon="book-outline" class="text-green" />
                <span class="is-hidden-mobile">บทความ</span>
              </a>
            </li>
            <li>
              <a @click="goto('top')">
...
export default {
  ...
  async fetch() {
    this.shortstories = await fetch(
      `${this.$axios.defaults.baseURL}api/read/sstoriescat/index/`
    )
      .then((res) => res.json())
      .catch((e) => {
        return []
    ...
    this.article = await fetch(
      `${this.$axios.defaults.baseURL}api/read/article/index/`
    )
      .then((res) => res.json())
      .catch((e) => {
        return []
